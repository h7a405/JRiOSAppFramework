    ...
    func initScrollView() {
        ...
        if self.scrollView == nil {
            self.scrollView = UIScrollView(frame: CGRect(
                ...
                width: self.viewWidth,
                height: self.isHeaderNeeded ? self.viewHeight - CGFloat(self.heightOfHeader) : self.viewHeight))
    ...
    func initPageControl() {
        ...
        if self.pageControl == nil {
            self.pageControl = UIPageControl(frame: CGRect(
                x: 0,
                y: self.viewHeight - CGFloat(self.heightOfPageControl) - 5,
                width: self.viewWidth,
...
extension JRCarouselView {
    func setupCarouselView(content: [AnyObject]?, titles: [String]?) {
        ...
        self.scrollView!.contentSize = CGSize(width: CGFloat(self.contents!.count) * self.scrollView!.viewWidth, height: self.scrollView!.viewHeight)
        if self.isHeaderNeeded {
            if titles != nil {
    ...
    func setupWithImages(images: [UIImage]) {
        for (index, image) in images.enumerate() {
            ...
            tempImageView.frame = CGRect(
                x: CGFloat(index) * self.scrollView!.viewWidth,
                y: 0,
                width: self.scrollView!.viewWidth,
                height: self.scrollView!.viewHeight)
    ...
    func setupHeader(titles: [String]) {
        let averageWidth: Float = Float(self.headerView!.viewWidth) / Float(titles.count)
        ...
        for (index, title) in titles.enumerate() {
            let tempButton: UIButton = UIButton(frame: CGRect(
                ...
                height: self.headerView!.viewHeight - CGFloat(self.heightOfHeaderIndicatorBar)))
...
//            tempButton.backgroundColor = UIColor.lightGrayColor()
        ...
        if self.selectedBar == nil {
            self.selectedBar = UILabel(frame: CGRect(
                ...
                y: self.headerView!.viewHeight - CGFloat(self.heightOfHeaderIndicatorBar) - 64,
                width: CGFloat(averageWidth),
    ...
    func setContentToShow(index: Int) {
        ...
        if self.headerView != nil {
            UIView.animateWithDuration(0.2, animations: {()
                self.selectedBar!.viewX = CGFloat(self.selectedIndex) * (self.headerView!.viewWidth / CGFloat(self.headerButtons!.count))
                self.headerButtons![self.selectedIndex].setTitleColor(self.headerSelectedColor, forState: UIControlState.Normal)
            })
        }
        self.scrollView!.setContentOffset(CGPoint(x: CGFloat(self.selectedIndex) * self.scrollView!.viewWidth, y: 0), animated: true)
...
extension JRCarouselView: UIScrollViewDelegate {
    func scrollViewDidEndDragging(scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        let currentOffset: CGPoint = CGPoint(x: CGFloat(currentIndex) * self.scrollView!.viewWidth, y: 0)
