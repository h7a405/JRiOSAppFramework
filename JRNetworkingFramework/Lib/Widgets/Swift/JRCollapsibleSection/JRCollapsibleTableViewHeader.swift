...
class JRCollapsibleTableViewHeader: UIView {
    ...
    func setupView() {
        ...
        if self.indicatorButton == nil {
            ...
            self.indicatorButton!.viewCenterVertical = self.viewCenterVertical
...
//            self.indicatorButton!.layer.cornerRadius = self.indicatorButton!.viewWidth / 2
...
//            self.indicatorButton!.layer.borderColor = UIColor.lightGrayColor().CGColor
        ...
        if self.textLabel == nil {
            self.textLabel = UILabel(frame: CGRect(x: 0, y: 0, width: 120, height: self.viewHeight))
...
//                self.selectingButton?.viewCenterVertical = self.centerVertical(self.selectingButton!).origin.y
                self.selectingButton?.viewCenterVertical = self.viewCenterVertical
            ...
            if self.multipleSelectedImage != nil && self.multipleUnselectedImage != nil {
                ...
                self.selectingButton!.layer.cornerRadius = self.selectingButton!.viewWidth / 2
        ...
        if self.style == JRCollapsibleTableViewHeaderStyle.Value1 {
            if self.detailTextLabel == nil {
                self.detailTextLabel = UILabel(frame: CGRect(x: 0, y: 0, width: 120, height: self.viewHeight))
        ...
        self.indicatorButton!.viewX = self.viewWidth - self.indicatorButton!.viewWidth - 5
        
        var centerWidth: CGFloat = self.indicatorButton!.viewX
        if self.selectionStyle == JRCollapsibleTableViewHeaderSelectionStyle.Mutilple {
            centerWidth -= CGRectGetMaxX(self.selectingButton!.frame)
            self.textLabel!.viewX = CGRectGetMaxX(self.selectingButton!.frame) + 5
        } else {
            self.textLabel!.viewX = 10
        }
        
        if self.style == JRCollapsibleTableViewHeaderStyle.Value1 {
            self.textLabel!.viewWidth = centerWidth / 2
            self.detailTextLabel!.viewWidth = centerWidth / 2
            self.detailTextLabel!.viewX = CGRectGetMaxX(self.textLabel!.frame) - 10
        } else {
            self.textLabel!.viewWidth = centerWidth
        }
