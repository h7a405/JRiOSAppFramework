...
class JRCalendarView: UIView, JRCalendarPickerViewDelegate {
    ...
    func setupTitle() {
        self.titleView.frame = CGRect(x: 0, y: 0, width: self.viewWidth, height: 40)
        
        self.titleLabel.frame = CGRect(x: (self.titleView.viewWidth / 3), y: 0, width: (self.titleView.viewWidth / 3), height: self.titleView.viewHeight)
        
        self.previousMonthButton.frame = CGRect(x: 10, y: 0, width: (self.titleView.viewWidth / 4) - 10, height: self.titleLabel.viewHeight)
        self.followingMonthButton.frame = CGRect(x: self.titleView.viewWidth - self.previousMonthButton.viewWidth - 10, y: 0, width: self.previousMonthButton.viewWidth - 10, height: self.titleLabel.viewHeight)
    ...
    func setupHeader() {
        self.headerView.frame = CGRect(x: 0, y: CGRectGetHeight(self.titleView.frame), width: self.viewWidth, height: 25)
        
        let array_strings = ["日", "一", "二", "三", "四", "五", "六"]
        
        for i in 0..<array_strings.count {
            let width = CGFloat(self.headerView.viewWidth / 7)
            
            let tempLabel = UILabel(frame: CGRect(x: width * CGFloat(i), y: 0, width: width, height: self.headerView.viewHeight))
...
//            tempLabel.layer.borderColor = self.titleLabel.textColor!.CGColor
        ...
        let dividerLabel = UILabel(frame: CGRect(x: 5, y: self.headerView.viewHeight - 1, width: self.headerView.viewWidth - 10, height: 1))
    ...
    func setupCalendar() {
        self.calendarView.frame = CGRect(x: 0, y: CGRectGetHeight(self.titleView.frame), width: self.viewWidth, height: (self.viewHeight - CGRectGetMaxY(self.headerView.frame)))
...
//        self.datePicker.frame = CGRect(x: self.datePicker.viewX, y: SCREEN_HEIGHT(), width: self.datePicker.viewWidth, height: 216)
        self.datePicker.addSubview(JRMonthPicker(frame: CGRect(x: 0, y: self.datePicker.viewHeight - 216, width: UIScreen.mainScreen().screenWidth(), height: 216), minYear: 2000, maxYear: 2050, delegate: self))
        self.datePicker.transform = CGAffineTransformTranslate(self.datePicker.transform, 0, self.datePicker.viewHeight)
    }
