...
class JRPickerView: UIView {
    ...
    convenience init(pickerData: [String]?, delegate: JRPickerViewDelegate?) {
        ...
        self.contentView.frame = CGRect(x: 0, y: self.viewHeight - 266, width: self.viewWidth, height: 266)
        self.contentView.backgroundColor = UIColor.blackColor()
        
        let cancelButton: UIButton = UIButton(frame: CGRect(x: 10, y: 0, width: self.contentView.viewWidth / 5, height: 50))
        ...
        let confirmButton: UIButton = UIButton(frame: CGRect(x: self.contentView.viewWidth - cancelButton.viewWidth - 10, y: 0, width: cancelButton.viewWidth, height: cancelButton.viewHeight))
        ...
        self.selectedContentLabel.frame = CGRect(x: (self.contentView.viewWidth - 120) / 2, y: 0, width: 120, height: 50)
        ...
        self.pickerView.frame = CGRect(x: 0, y: 50, width: self.contentView.viewWidth, height: 216)
    ...
    func resizeComponents() {
        self.coverView.frame = self.bounds
        self.contentView.frame = CGRect(x: 0, y: self.viewHeight - 256, width: self.viewWidth, height: 256)
    }
    ...
    func showOnView(superView: UIView) {
        ...
        self.coverView.alpha = 0
        self.contentView.transform = CGAffineTransformTranslate(self.contentView.transform, 0, self.contentView.viewHeight)
...
extension JRPickerView: UIPickerViewDataSource {
    ...
    func pickerView(pickerView: UIPickerView, viewForRow row: Int, forComponent component: Int, reusingView view: UIView?) -> UIView {
        if view == nil {
            let newView: UILabel = UILabel(frame: CGRect(x: 0, y: 0, width: pickerView.viewWidth, height: 40.0))
