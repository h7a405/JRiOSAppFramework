...
//            self.footerView.viewY += CGFloat(yToTransform)
//            self.webView.viewHeight += CGFloat(yToTransform)
...
extension JRWebViewController: UIWebViewDelegate {
    func webView(webView: UIWebView, didFailLoadWithError error: NSError?) {
        ...
        self.progressBar.viewWidth = self.headerView.frame.width
    }
    func webViewDidStartLoad(webView: UIWebView) {
        ...
        self.progressBar.viewWidth = 0
        UIView.animateWithDuration(1, animations: {()
            self.progressBar.viewWidth = self.headerView.frame.width * 0.75
        }, completion: nil)
    }
