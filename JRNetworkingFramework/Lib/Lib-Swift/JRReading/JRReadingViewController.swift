    ...
    var toolBarHidden: Bool = false {
        willSet {
            if self.toolBarHidden != newValue {
                ...
                if newValue {
                    yToTransform = self.toolBar.viewHeight
                } else {
                    yToTransform = -(self.toolBar.viewHeight)
                }
    ...
    func setupReadingViews() {
        if self.readingViews != nil {
            for i in 0..<self.readingViews!.count {
                readingViews![i].frame = CGRect(x: CGFloat(i) * self.contentScrollView.viewWidth, y: 0, width: self.contentScrollView.viewWidth, height: self.contentScrollView.viewHeight)
                readingViews![i].backgroundColor = UIColor.colorWithRed( CGFloat(i) * 50, green: 100, blue: 100)
                self.contentScrollView.addSubview(readingViews![i])
            }
        }
        self.contentScrollView.contentSize = CGSize(width: CGFloat(self.readingViews!.count) * self.contentScrollView.viewWidth, height: self.contentScrollView.viewHeight)
//        self.contentScrollView.setContentOffset(CGPoint(x: CGFloat(Int(self.readingViews!.count)) * self.contentScrollView.viewWidth, y: 0), animated: false)
