...
extension HttpViewController: JRNetworkingApiManagerProtocol {
    func ApiManager(ApiManager: JRNetworkingApiManager, performWithDicionary dic: NSDictionary?) {
        if dic == nil {
            self.resultTextView.text = String(DATA_REQUEST_FAIELD)
        } else {
            self.resultTextView.text = String("\(dic)")
        }
    }
    func ApiManager(ApiManager: JRNetworkingApiManager, performWithError error: NSError!) {
        self.resultTextView.text = String(NETWORK_REQUEST_FAILED)
    }
}
extension HttpViewController: JRNetworkingViewContollerProtocol {
    func requestData() {
        let theURL: String = "http://" + self.baseURLTextField.text!
        ...
        var objects: [String] = Array()
        for (_, (key, object)) in (self.dataOfParamters!).enumerate() {
            if !key.isEmpty && !object.isEmpty {
                keys.append(key)
                objects.append(object)
            }
        }
        let apiManager: JRNetworkingApiManager = JRNetworkingApiManager(delegate: self)
        apiManager.apiHttpTest(method!, url: theURL, apiInterface: self.routeTextField.text, keys: keys, objects: objects)
    }
}
