...
extension JRRatingView {
    ...
    func setupRatingView() {
        for (index, button) in self.buttonArray.enumerate() {
            let averageWidthOfButton: CGFloat = self.widthOfFrame / CGFloat(self.totalNumberOfRatingStars)
            button.frame = CGRect(x: CGFloat(index) * averageWidthOfButton, y: 0, width: averageWidthOfButton, height: self.heightOfFrame)
        }
    }
}
